...
from keras.layers import InputLayer, GRU , Bidirectional , LSTM , Dense
...
model2.add(Dense(1, 'linear'))  # add a Dense layer with 1 output unit
model2.summary()
...
history = model2.fit(X_train1, y_train1, epochs=50, batch_size=64, validation_split=0.2, verbose=1)


# In[27]:


y_train_pred = model2.predict(X_train1).flatten()

train_mse = mean_squared_error(y_train1, y_train_pred)
train_rmse = np.sqrt(train_mse)

y_test_pred = model2.predict(X_test1).flatten()
test_mse = mean_squared_error(y_test1, y_test_pred)
test_rmse = np.sqrt(test_mse)
